...
TMP_CHANGELOG_FILE = "/tmp/braintree-android-visa-checkout-release.md"

task :default => :unit_tests

desc "Run Android lint on all modules"
task :lint do
  sh "./gradlew clean lint :VisaCheckout:assembleDebug :VisaCheckout:assembleRelease"
end
...
task :release => :unit_tests do
  ...
  Rake::Task["release_braintree_visa_checkout"].invoke
...
def prompt_for_change_log(version)
  ...
  IO.write("CHANGELOG.md",
    File.open("CHANGELOG.md") do |file|
      file.read.gsub("# Braintree Android Visa Checkout SDK Release Notes\n", "# Braintree Android Visa Checkout SDK Release Notes\n\n## #{new_changes.chomp}")
    end
  )
end
...
def post_release(version)
  if !`git remote`.include?("github")
    sh "git remote add github https://github.com/braintree/braintree_android_visa_checkout.git"
  end
...
def update_readme_snapshot_version(snapshot_version)
  IO.write("README.md",
    File.open("README.md") do |file|
      file.read.gsub(/:braintree:\d+\.\d+\.\d+-SNAPSHOT'/, ":braintree-visa-checkout:#{snapshot_version}-SNAPSHOT'")
    end
  )
end
